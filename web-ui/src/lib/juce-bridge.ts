/**
 * JUCE 8 WebView Bridge for Svelte
 *
 * Provides access to JUCE's native relay system for parameter binding.
 */
...
export function addCustomEventListener(event: string, callback: EventCallback): () => void {
  if (!eventListeners.has(event)) {
    eventListeners.set(event, new Set());

    // Register with JUCE backend if available
